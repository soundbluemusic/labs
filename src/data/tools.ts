...
export const tools: Tool[] = [
  ...
  },
  {
    ...
    description: {
      ko: "도구1(Claude 버전)로 생성한 대화들을 모아, 원어민 심사로 자연스럽고 반응이 살아있는 8점 이상만 엄선한 녹음 대본입니다. 모든 발음(초성·중성·종성)이 골고루 들어가도록 음소 균형을 맞춰 약 30분 분량으로 구성했습니다. 소리 내어 읽어 음성 복제(TTS) 모델 학습용 녹음 대본으로 바로 쓸 수 있고, 로그인·인터넷 없이 보고 텍스트로 내려받을 수 있습니다. 새 대화가 쌓일수록 더 엄격하게 갱신됩니다.",
      en: "A recording script assembled from dialogues generated by Tool 1 (the Claude version), keeping only the natural, genuinely reactive ones (score 8+) chosen by native-speaker review. It is phoneme-balanced (onset/nucleus/coda) and runs about 30 minutes. Read it aloud as a recording script for TTS / voice-cloning model training; view and download it as plain text with no login and no internet. It is refined further as more dialogues accumulate.",
      ja: "ツール1(Claude版)で生成した対話を集め、ネイティブ審査で自然かつ反応が生きている8点以上だけを厳選した録音台本です。すべての発音(初声・中声・終声)が均等に入るよう音素バランスを取り、約30分分で構成しています。音読してTTS・音声クローニング学習用の録音台本としてそのまま使え、ログイン・インターネット不要で閲覧・テキストダウンロードできます。対話が貯まるほど厳しく更新されます。",
    },
    category: {
      ko: "음성 · 오디오",
      en: "Voice · Audio",
